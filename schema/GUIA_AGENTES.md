...
El prefijo de categoría en el ID **no cambia** con esta actualización — sigue identificando la categoría clínica de la carta, independientemente de en qué set (mixto) se publique.
...
10. `meta.categoria_clinica` es OBLIGATORIO y debe coincidir con el prefijo del `id` (ej. un ID `BACT-*` debe tener `categoria_clinica: "Bacterias"`).
...
│       └── FARM-COMP-001_fluconazol.json          (a regenerar bajo schema v1.1)
├── data/
│   ├── SET-01/
